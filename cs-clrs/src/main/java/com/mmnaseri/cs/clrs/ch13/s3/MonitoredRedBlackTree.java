...
public class MonitoredRedBlackTree<E> extends RedBlackTree<E> {
  ...
  public MonitoredRedBlackTree(Comparator<E> comparator) {
    super(comparator);
    monitor =
        controlDataStructure(this)
            .givenFeature("color")
            .using(new RedBlackTreeColorController<>())
            .andFeature("black root")
            .using(new RedBlackTreeRootColorController<>())
            .andFeature("red node has black children")
            .using(new RedBlackTreeLevelColorController<>())
            .andFeature("black height")
            .using(new RedBlackTreeBlackHeightController<>())
            .viaMonitor();
    monitor.validate();
  }
  ...
  private static class RedBlackTreeColorController<E>
      implements FeatureController<MonitoredRedBlackTree<E>> {

    @Override
    public Set<Failure<MonitoredRedBlackTree<E>>> control(
        final MonitoredRedBlackTree<E> dataStructure) {
      ...
      walk.perform(
          ...
          node -> {
            if (!NodeColor.BLACK.equals(node.getColor())
                && !NodeColor.RED.equals(node.getColor())) {
              failures.add(
                  new Failure<>(
                      node,
                      dataStructure,
                      "Expected node color to be either RED or BLACK but it is "
                          + node.getColor()));
            }
          });
      return failures;
  ...
  private static class RedBlackTreeRootColorController<E>
      implements FeatureController<MonitoredRedBlackTree<E>> {

    @Override
    public Set<Failure<MonitoredRedBlackTree<E>>> control(MonitoredRedBlackTree<E> dataStructure) {
      if (dataStructure.getRoot() != null
          && !NodeColor.BLACK.equals(dataStructure.getRoot().getColor())) {
        return Collections.singleton(
            new Failure<>(
                dataStructure.getRoot(),
                dataStructure,
                "Expected the root to be black but it was " + dataStructure.getRoot().getColor()));
      }
      return null;
    }
  }

  private static class RedBlackTreeLevelColorController<E>
      implements FeatureController<MonitoredRedBlackTree<E>> {
          ...
          node -> {
            if (NodeColor.RED.equals(node.getColor())) {
              if (node.getLeftChild() != null
                  && !NodeColor.BLACK.equals(node.getLeftChild().getColor())) {
                failures.add(
                    new Failure<>(
                        node,
                        dataStructure,
                        "Expected the child of RED node to be black but its left child was "
                            + node.getLeftChild().getColor()));
              }
              if (node.getRightChild() != null
                  && !NodeColor.BLACK.equals(node.getRightChild().getColor())) {
                failures.add(
                    new Failure<>(
                        node,
                        dataStructure,
                        "Expected the child of RED node to be black but its right child was "
                            + node.getRightChild().getColor()));
              }
            }
          });
      return failures;
  ...
  private static class RedBlackTreeBlackHeightController<E>
      implements FeatureController<MonitoredRedBlackTree<E>> {
    ...
    public Set<Failure<MonitoredRedBlackTree<E>>> control(MonitoredRedBlackTree<E> dataStructure) {
      ...
      for (RedBlackTreeNode<E> leaf : leaves) {
        int blacks =
            0; // whether it is a red leaf with black `null` children or an actual `black` leaf we
        // count it as one
        ...
        if (expected < 0) {
          ...
          failures.add(
              new Failure<>(
                  leaf,
                  dataStructure,
                  "Expected black height of leaf to be " + expected + " but it was " + blacks));
        }
      }
      return failures;
    }
