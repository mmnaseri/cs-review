    ...
    public BinarySearchTreeSpecification find(double[] hits, double[] misses) {
        ...
        for (int length = 1; length <= hits.length; length ++) {
            for (int i = 1; i <= misses.length - length; i ++) {
                ...
                specification.set(i, j, new SplitSpecification(-1, Integer.MAX_VALUE, specification.get(i, j - 1).getWeight() + hits[j - 1] + misses[j]));
                for (int root = i; root <= j; root ++) {
                    ...
                    if (expectation < specification.get(i, j).getExpectation()) {
                        specification.set(i, j, new SplitSpecification(root - 1, expectation, specification.get(i, j).getWeight()));
                    }
                }
            }
        }
