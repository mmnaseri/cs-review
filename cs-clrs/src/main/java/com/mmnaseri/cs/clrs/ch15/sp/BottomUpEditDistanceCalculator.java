...
@Quality(
    value = Stage.TESTED,
    explanation =
        "the whole spec mess is because we want the actual operations and not just the distances")
public class BottomUpEditDistanceCalculator implements EditDistanceCalculator {
  ...
  private void calculateDistance(
      String source, String target, Specification[][] specifications, CostFunction function) {
    for (int j = 1; j < target.length() + 1; j++) {
      for (int i = 1; i < source.length() + 1; i++) {
        ...
        if (source.charAt(i - 1) == target.charAt(j - 1)) {
          final int functionCost =
              function.getCost(EditOperationType.COPY, source, target, i - 1, j - 1);
          final int previousCost = specifications[i - 1][j - 1].getCost();
          minimum =
              new Specification(EditOperationType.COPY, previousCost + functionCost, functionCost);
        }
        if (i >= 2
            && j >= 2
          ...
          final int functionCost =
              function.getCost(EditOperationType.TWIDDLE, source, target, i - 2, j - 2);
          final int previousCost = specifications[i - 2][j - 2].getCost();
          if (functionCost + previousCost < minimum.getCost()) {
            minimum =
                new Specification(
                    EditOperationType.TWIDDLE, previousCost + functionCost, functionCost);
          }
        }
        for (EditOperationType type :
          ...
          final int functionCost =
              function.getCost(type, source, target, i - type.getSource(), j - type.getTarget());
          final int previousCost =
              specifications[i - type.getSource()][j - type.getTarget()].getCost();
          if (functionCost + previousCost < minimum.getCost()) {
  ...
  private List<EditOperation> constructEditOperations(
      String source, String target, Specification[][] specifications) {
  ...
  private Specification[][] prepareDistanceMatrix(
      String source, String target, CostFunction function) {
