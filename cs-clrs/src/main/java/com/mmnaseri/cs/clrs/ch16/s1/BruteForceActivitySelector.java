...
public class BruteForceActivitySelector implements ActivitySelector {
    ...
    @Override
    @Complexity(value = "O(n^2 * 2^n)", explanation = "`findProperCandidate` is the most significant operation")
    public Set<Integer> select(Activity... activities) {
        //we first note the index of the activities so that after sorting we can recall where each item was
    ...
    @Complexity(value = "O(n^2 * 2^n)", explanation = "There are 2^n subsequences and each one is validated using O(n^2) compatibility control")
    ...
    @Complexity(value = "O(2^n * nlgn)", explanation = "Each subsequence is of size order O(n) and merge sorting it requires O(nlgn) and there are 2^n such lists")
    ...
    @Complexity(value = "O(n2^n)", explanation = "We are sorting a collection of size 2^n using merge sort which is O(2^nlg(2^n)) = O(n2^n)")
    private void sortAllSubSequences(List<List<IndexedActivity>> sequences) {
        Collections.sort(sequences, new Comparator<List<IndexedActivity>>() {
            ...
            public int compare(List<IndexedActivity> first, List<IndexedActivity> second) {
                ...
                if (comparison == 0) {
                    //our algorithm should go for the sequence that finishes sooner (which leaves the most amount of unused space)
                    return Integer.compare(first.get(first.size() - 1).getFinish(), second.get(second.size() - 1).getFinish());
                }
                return comparison;
            }
        });
    ...
    @Complexity(value = "O(2^n)", explanation = "There are 2^n possible subsets for each set of size n, and we are generating them all dynamically")
    ...
    @Complexity(value = "O(n^2)", explanation = "Each two activities are compared once: O(selection 2 of n) = O(n^2)")
