...
public class HuffmanCodeBuilder {
    ...
    public List<CharacterCode> build(CharacterComponent... components) {
        ...
        final MinHeapProperty<ComparableBinaryTreeNode<CharacterComponent>> heapProperty = new MinHeapProperty<>();
        final Heap<ComparableBinaryTreeNode<CharacterComponent>> heap = new ArrayHeap<>(heapProperty, nodes);
        while (heap.size() > 1) {
            ...
            root.setValue(new CharacterComponent(null, left.getValue().getFrequency() + right.getValue().getFrequency()));
            heap.add(root);
        }
        if (heap.isEmpty()) {
            return result;
        ...
        final PreOrderTreeWalk<CharacterComponent, ComparableBinaryTreeNode<CharacterComponent>> walk = new PreOrderTreeWalk<>();
        final List<ComparableBinaryTreeNode<CharacterComponent>> leaves = walk.perform(root, new Walker()).getLeaves();
        for (ComparableBinaryTreeNode<CharacterComponent> leaf : leaves) {
            ...
            result.add(new CharacterCode(character, code.toArray(new Bit[code.size()])));
        }
        return result;
    }
    ...
    private static class ComparableBinaryTreeNode<E extends Comparable<E>> extends BinaryTreeNode<E> implements Comparable<ComparableBinaryTreeNode<E>> {
        ...
        @Override
        public int compareTo(@SuppressWarnings("NullableProblems") ComparableBinaryTreeNode<E> that) {
            return getValue().compareTo(that.getValue());
        }

    }

    private static class Walker implements TreeWalkCallback<CharacterComponent, ComparableBinaryTreeNode<CharacterComponent>> {
